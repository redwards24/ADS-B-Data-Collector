...
public class Flight implements Displayable<Flight>
{
	...
	private String toDateTime(final long time)
	{
		return LocalDateTime.ofInstant(Instant.ofEpochSecond(time), ZoneOffset.UTC).format(formatter);
	}
	
	private int toUnixTimestamp(final String time)
	{
		return (int)LocalDateTime.parse(time, formatter).atZone(ZoneId.of("UTC")).toEpochSecond();
	}
	...
	public Object[] toArray()
	{
		return new Object[] 
		{
			this.icao24, this.callsign, toDateTime(this.start), toDateTime(this.stop), this.messageCount, this.type, this.source
		};
	}
	...
	public Flight toObject(final JTable table, final int row)
	{
		try
		{
			...
			(
				...
				toUnixTimestamp((String)table.getValueAt(row, 2)),
				toUnixTimestamp((String)table.getValueAt(row, 3)),
